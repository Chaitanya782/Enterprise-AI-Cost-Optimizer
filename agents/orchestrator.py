"""Enhanced Multi-Agent Orchestrator with FIXED intent classification and better keyword matching"""
...
class Orchestrator:
    """Enhanced Orchestrator with FIXED intent classification and better keyword matching"""

    # FIXED: Enhanced intent keywords with better classification
    INTENT_KEYWORDS_ENHANCED = {
        "cost_analysis": {
            # High priority cost terms (weight 3x)
            "high_priority": [
                "cost", "price", "pricing", "budget", "expensive", "cheap", "spend", "fee", "charge",
            ...
            "medium_priority": [
                "compare llm", "llm pricing", "model cost", "provider cost", "usage cost",
                "cost of implementing", "budget for", "expense", "financial", "cost effective",
                "save money", "reduce cost", "cost reduction", "cheaper alternative"
            ],
            # Context cost terms (weight 1x)
            "context": [
                "gpt-4 cost", "claude cost", "gemini cost", "openai pricing", "anthropic pricing",
                "token cost", "request cost", "volume pricing", "enterprise pricing"
            ]
        },
        "task_analysis": {
            "high_priority": [
            ...
            "medium_priority": [
                ...
                "productivity", "workflow optimization", "process improvement", "automation opportunities"
            ],
            "context": [
                "automation", "digitize", "transform", "manual process", "repetitive task",
        ...
        "roi_analysis": {
            "high_priority": [
                ...
                "roi analysis", "roi calculation", "investment return", "payback period"
            ],
            "medium_priority": [
                "worth", "profit", "savings", "break even", "business case", "justification",
                "cost benefit", "financial return", "investment analysis", "benefit analysis"
            ],
            "context": [
                "cost benefit analysis", "financial justification", "business value",
                "investment payback", "return calculation"
            ]
        }
    }
    ...
    def _classify_intent(self, user_query: str, session_id: str = None) -> Tuple[str, float]:
        """FIXED: Enhanced intent classification with better keyword matching and scoring"""
        ...
        # FIXED: Enhanced weighted scoring with phrase matching
        for intent, keyword_groups in self.INTENT_KEYWORDS_ENHANCED.items():
            score = 0

            # High priority keywords (3x weight) - FIXED: Better phrase matching
            for keyword in keyword_groups.get("high_priority", []):
                if keyword in query_lower:
                    # Give extra weight for exact phrase matches
                    phrase_weight = len(keyword.split()) if ' ' in keyword else 1
                    score += 3 * phrase_weight

            # Medium priority keywords (2x weight)
            for keyword in keyword_groups.get("medium_priority", []):
                if keyword in query_lower:
                    phrase_weight = len(keyword.split()) if ' ' in keyword else 1
                    score += 2 * phrase_weight

            # Context keywords (1x weight)
            for keyword in keyword_groups.get("context", []):
                if keyword in query_lower:
                    phrase_weight = len(keyword.split()) if ' ' in keyword else 1
                    score += 1 * phrase_weight

            weighted_scores[intent] = score

        # FIXED: Special case handling for specific triggers
        # Cost analysis triggers
        cost_triggers = ["cost", "price", "pricing", "budget", "compare", "comparison"]
        model_names = ["gpt", "claude", "gemini", "llama", "openai", "anthropic"]
        
        has_cost_trigger = any(trigger in query_lower for trigger in cost_triggers)
        has_model_name = any(model in query_lower for model in model_names)
        
        if has_cost_trigger and has_model_name:
            weighted_scores["cost_analysis"] += 10  # Strong boost

        # ROI analysis triggers
        roi_triggers = ["roi", "return", "investment", "payback", "benefit", "worth"]
        if any(trigger in query_lower for trigger in roi_triggers):
            weighted_scores["roi_analysis"] += 8

        # Task analysis triggers
        task_triggers = ["automate", "automation", "workflow", "process", "task", "streamline"]
        if any(trigger in query_lower for trigger in task_triggers):
            weighted_scores["task_analysis"] += 6
        ...
        if total_score == 0:
            ...
            confidence = min(1.0, max_intent[1] / max(total_score, 1))

            # FIXED: Enhanced confidence threshold and comprehensive detection
            ...
            if len(active_intents) > 1:
                ...
                # FIXED: Better comprehensive detection
                if has_comprehensive_intent and confidence < 0.7:
                    result = ("comprehensive", confidence)
                elif len(active_intents) >= 3:  # Multiple strong intents
                    result = ("comprehensive", confidence)
                else:
    ...
    def _extract_financial_metrics(self, query: str, contextual_defaults: dict = None) -> dict:
        """FIXED: Enhanced financial data extraction with better regex patterns"""
        metrics = {}
        ...
        for i, pattern in enumerate(self.FINANCIAL_PATTERNS):
            for match in re.finditer(pattern, query_lower, re.IGNORECASE):
                ...
                # FIXED: Check for multiplier with proper null handling
    ...
    def _get_dynamic_use_case_defaults(self, query: str, use_case: str) -> Dict[str, Any]:
        ...
        if any(indicator in query_lower for indicator in ["detailed", "complex", "comprehensive"]):
            complexity_multiplier = 2.0
        elif any(indicator in query_lower for indicator in ["simple", "basic", "quick"]):
        ...
        if any(indicator in query_lower for indicator in ["enterprise", "large scale", "high volume"]):
            volume_multiplier = 3.0
        elif any(indicator in query_lower for indicator in ["small", "startup", "limited"]):
